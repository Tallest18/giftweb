  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-white/95 backdrop-blur-md shadow-sm' : 'bg-transparent'}`}>
      ...
      <div className="bg-charcoal-dark text-cream-100 text-center text-xs py-2 font-body tracking-widest">
        🌍 WORLDWIDE SHIPPING &nbsp;·&nbsp; FREE DELIVERY IN CANADA ON ORDERS OVER CA$100 &nbsp;·&nbsp; SAME-DAY DELIVERY IN TORONTO, VANCOUVER & MONTREAL
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          <div className="hidden md:flex items-center gap-8">
            {links.map(l => (
              <Link key={l.to} to={l.to}
                className={`font-body text-sm font-medium transition-colors relative group ${location.pathname === l.to ? 'text-[#C4687A]' : 'text-charcoal-mid hover:text-[#C4687A]'}`}>
                {l.label}
          ...
          <div className="flex items-center gap-1">
            <CurrencySelector />
            <button onClick={() => setSearchOpen(!searchOpen)} className="p-2.5 text-charcoal-mid hover:text-[#C4687A] transition-colors rounded-full hover:bg-cream-100">
              <Search size={19} />
            </button>
            <Link to="/wishlist" className="p-2.5 text-charcoal-mid hover:text-[#C4687A] transition-colors rounded-full hover:bg-cream-100">
              <Heart size={19} />
            </Link>
            <Link to="/cart" className="relative p-2.5 text-charcoal-mid hover:text-[#C4687A] transition-colors rounded-full hover:bg-cream-100">
              ...
              {itemCount > 0 && (
                <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }}
                  className="absolute -top-0.5 -right-0.5 w-5 h-5 bg-[#C4687A] text-white text-xs font-bold rounded-full flex items-center justify-center">
      ...
      <AnimatePresence>
        {searchOpen && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
            className="bg-white border-t border-cream-200 overflow-hidden">
            <div className="max-w-2xl mx-auto px-4 py-3">
              <div className="relative">
                <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-charcoal-light" />
                <input autoFocus type="text" placeholder="Search for flowers, hampers, jewellery..."
        ...
        {menuOpen && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
            ...
            <div className="px-4 py-4 space-y-1">
              {links.map(l => (
                <Link key={l.to} to={l.to}
                  className={`block px-4 py-3 rounded-lg font-body font-medium text-sm transition-colors ${location.pathname === l.to ? 'bg-cream-100 text-[#C4687A]' : 'text-charcoal-mid hover:bg-cream-50 hover:text-[#C4687A]'}`}>
                  {l.label}
                </Link>
              ))}
            </div>
