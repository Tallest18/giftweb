...
const ProductCard: React.FC<Props> = ({ product, index = 0 }) => {
  ...
  const handleAdd = (e: React.MouseEvent) => {
    ...
    toast.success(`${product.name} added! 🎁`, {
      style: { background: '#1C1C2E', color: '#FAF4EC', fontFamily: 'DM Sans', borderRadius: '12px' },
    });
      ...
      <Link to={`/product/${product.id}`} className="block">
        <div className="img-zoom-container relative h-56 bg-cream-100">
          ...
          <button onClick={e => { e.preventDefault(); e.stopPropagation(); setWished(!wished); }}
            className="absolute top-3 right-3 w-9 h-9 bg-white/90 rounded-full flex items-center justify-center shadow-sm hover:scale-110 transition-transform z-10">
            <Heart size={15} className={wished ? 'fill-[#C4687A] text-[#C4687A]' : 'text-charcoal-light'} />
          </button>
        </div>
        <div className="p-4">
          ...
          <div className="flex items-center gap-1.5 mb-3">
            <div className="flex">
              {[1,2,3,4,5].map(s => <Star key={s} size={11} className={s <= Math.floor(product.rating) ? 'fill-[#D4A843] text-[#D4A843]' : 'text-cream-300 fill-cream-300'} />)}
            </div>
            <span className="text-xs text-charcoal-light font-body">({product.reviews})</span>
          </div>
          <div className="flex items-center justify-between">
            <div className="flex items-baseline gap-2">
              <span className="font-display text-lg font-bold text-charcoal-dark">{formatPrice(product.price, currency)}</span>
              {product.originalPrice && (
                <span className="text-sm text-charcoal-light line-through font-body">{formatPrice(product.originalPrice, currency)}</span>
              )}
            </div>
